...
description: >-
  ...
  branch's code changes are accompanied by CHANGELOG/README/docs updates. Runs
...
- A lightweight verification script is provided at scripts/verify-docs-updated.sh. It checks the diff between the current branch and a base ref (default origin/main) and fails if code changes exist without accompanying CHANGELOG/README or other .md updates.
...
- When making changes that affect behavior, APIs, public interfaces, or user-visible output, update CHANGELOG.md (add a bullet under the top `### v<next> - Unreleased` section) and any affected docs.
...
- The script accepts a base-ref argument (e.g. a PR base ref).
